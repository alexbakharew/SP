...
display("+")return close__enough_Q(neg__point, pos__point) ? midpoint :
(test__value = fun(midpoint);
,
!((test__value <= 0)) ? __BAT2018__try(neg__point, midpoint) :
...
double root(double a, double b) {
display("interval=\t[")display(a)display(" , ")display(b)display("]\n"){//let
 double temp(half__interval__metod(a, b)), x(0);
newline()display("discrepancy=\t")display(fun(temp))newline()display("root=\t\t")x = (((temp - b) - one) == 0) ? display("[bad]")one :
(display("[good]"),
two);
return temp;
}//let
